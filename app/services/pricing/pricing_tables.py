...
field via ``category_mapping.py`` -- these tables themselves are keyed by
``feature_schema.CATEGORIES``' naming, not the raw DB value) -- not new schema
concepts, so nothing here needs a corresponding ERD/class-diagram change.

Ported near-verbatim from ``ml-experiments/pricing_tables.py`` (Phase 2a,
2026-08-11) -- see that file for the original Phase 1 authoring history.
``CATEGORY_BASE_RATE`` is kept for reference/tests only: production
``model.py`` clamps against real per-asset ``Asset.minDailyRate``/
``maxDailyRate`` passed in by the caller, never this static per-category
table (that was ``ml-experiments/predict_price.py``'s prototype stand-in).
"""
