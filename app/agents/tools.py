"""Haystack-backed tools for Stage-1 project knowledge agents + S3 index gate + S6 pricing.
...
Pricing tool [7] is in-process only — same entrypoint as service recommend path.
"""
...
TOOL_PREDICT_ASSET_PRICE = "predict_asset_price"
...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    TOOL_PREDICT_ASSET_PRICE: (
        "Pricing Worker [7]: in-process ML daily rate for one asset/window "
        "via pricing_client (production model + per-asset clamp). Returns "
        "daily_rate, total_price, clamp metadata, model_version. Never invents "
        "rates; never silent zeros. Not a public HTTP price API."
    ),
}
...
def predict_asset_price(
    *,
    ...
    """In-process pricing tool for Pricing Worker [7] (Phase 6 / S6).

    Single source of truth with the service recommend path: delegates to
    ``pricing_client.predict_price_for_asset`` (production model + per-asset
    clamp). Does not invent rates. Fails loud if the underlying path returns
    a non-positive ``daily_rate`` (silent zeros forbidden).

    ``asset_id`` is optional metadata for traces/synthesis; it is not used to
    load a second prediction path. Live ``period_utilization`` /
    ``lead_time_days`` are computed inside the model when ``db`` + dates are
    provided (same as pipeline / quote API).
    """
    ...
    if price.daily_rate <= 0:
        raise ValueError(
            f"predict_asset_price: silent zero forbidden "
            f"(daily_rate={price.daily_rate!r}, model_version={price.model_version!r})"
        )
