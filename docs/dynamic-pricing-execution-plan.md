...
- **Fix category-name mismatch** (found 2026-08-11 during Phase 2 prep, confirmed live against `heavy_rental`): `AssetCategory.name` (`Excavator`, `Scissors Lift`, `Boom Lift`, `Fork Lift`) never matches `feature_schema.CATEGORIES` (`excavator`, `scissor lift`, `boom lift`, `forklift`) — `compute_period_utilization()`'s join has silently always fallen back to the static per-category constant, no error, no degraded flag. Add a shared `DB_NAME_TO_FEATURE_NAME` mapping at the read boundary as part of this relocation (see spec/design), plus a test that exercises the real `WHERE` clause with DB-shaped names, not a fully mocked session. Full detail: `openspec/specs/dynamic-pricing/spec.md`/`design.md`, 2026-08-11.
...
- Wire the pricing function into the agentic pipeline → return the prediction in the recommendation response (`item.pricing.daily_rate`). No DB write: Spring Boot persists `RecommendationItem.mlPredictedPrice` on its side (locked 2026-08-11 — see masterplan)
...
| 3 | ☐ | Phase 2 — productionize pricing service | `feature/ml-3-pricing-service` | `app/services/pricing/` package, guardrail clamping, in-process `predict_price()` function, **category-name mismatch fix** (2026-08-11) | 4 |
| 4 | ☐ | Phase 2 — pipeline integration & tests | `feature/ml-4-integration-tests` | Wire into agentic pipeline → return prediction in response (no DB write — Spring Boot persists `mlPredictedPrice`, **locked**), unit tests, manual "retrain now" endpoint | 5 (AM) |
...
- [ ] **New (2026-08-11):** category-name mismatch fix — folded into Phase 2a (`feature/ml-3-pricing-service`), not a separate subtask. See spec/design change control 2.2.0.
