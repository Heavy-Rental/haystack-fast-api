...
- `purchaseYear` (int) — optional proxy for equipment age; only add if `condition` alone doesn't capture enough signal
...
- [ ] Exact feature list `feature_schema.py` maps from `Booking`/`BookingItem`/`Asset` — finalize after the first SHAP review in Phase 1.
