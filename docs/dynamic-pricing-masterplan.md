...
3. **Phase 1d** — real-time features (`period_utilization`, `lead_time_days`) added to `ml-experiments/` (still scratch, offline — same convention as Phase 1/1b/1c)
4. **Phase 1e** — minimal production DB wiring for `period_utilization`'s live query only: first-ever read-only SQLAlchemy `Asset`/`AssetCategory`/`Booking` models + a repository query, wired into the *existing* prototype call path (`pricing_client.py` → `ml-experiments/predict_price.py`). This is real production code touching the live pipeline (unlike 1/1c/1d), pulled forward ahead of the rest of Phase 2's package move — same "get ahead of Phase 2" pattern Phase 1c already established, just no longer scratch.
5. **→ Write `SPEC-dynamic-pricing.md` + `SPEC-domain-seed-data.md` here ←**
6. **Phase 2** — productionize into `app/services/pricing/` (relocates, doesn't rebuild, the Phase 1e repository logic — see that section)
...
- **`period_utilization` (Phase 1d): new numeric feature, the first real-time (not static-attribute) signal in the model.** Fraction of assets in the same `category` **+ spec-band** already booked (status `CONFIRMED`/`PENDING`, non-cancelled) over the requested rental window — a live aggregate query at prediction time, not a forecast. Grouping by raw `category` alone would be misleading (a fully-booked small-excavator fleet shouldn't make a large excavator look scarce), so utilization is computed at category + spec-band: excavator/forklift key off `capacity`, scissor lift/boom lift key off `platform_height`, via fixed-constant bins (`pricing_tables.CAPACITY_BINS`/`HEIGHT_BINS`, grounded in load-class/aerial-catalog-tier conventions — see the References section below) computed on the fly from the existing `capacity`/`platform_height` columns, **no new persisted band column**. **Important, not a bug**: an early booking on a not-yet-claimed window legitimately produces a lower `period_utilization` and often a lower predicted price — this is intentional scarcity pricing, the same mechanism airline/hotel pricing uses, not an early-bird pricing bug to "fix." **Band boundaries and band count are an implementation decision, not locked to any specific numbers** — early planning conversations floated illustrative example numbers for `CAPACITY_BINS`/`HEIGHT_BINS`, but those were explicitly not meant to be transcribed. Whoever implements Phase 1d chooses the actual boundaries, grounded in (a) the ranges already present in this repo's `CATEGORY_CAPACITY_KG`/`CATEGORY_PLATFORM_HEIGHT_M` (e.g. excavator capacity only spans 1000–30000 kg today — bands must meaningfully split *that* range, not an arbitrary external one) and (b) real-world equipment-rental load-class/aerial-height-tier conventions for how many bands and where the natural breakpoints fall. Sanity-check empirically once synthetic data exists (`generate_synthetic_data.py --strict`): every band should get a non-trivial share of rows for its affected categories — an empty/near-empty band means the boundaries need adjusting.
- **`lead_time_days` (Phase 1d): new numeric feature** — days between the request date (today) and `Booking.startDate`. Simple derived field, no new column. **Kept alongside `period_utilization` despite being correlated** — they answer different questions (is this specific window already claimed vs. how much notice did the booker give), and a Phase 1d SHAP review compares which one the model actually leans on, rather than assuming and dropping one upfront.
- **Fuel price: considered and rejected as a candidate feature (Phase 1d).** Three reasons: (1) it's an indirect/lagged signal — fuel cost → construction activity → demand, not a direct per-booking effect; (2) it would require a new external API dependency, which doesn't fit the current in-process/no-outbound-calls architecture; (3) on synthetic data specifically it would be untrainable noise unless the generator explicitly fabricates a fuel-price-to-price correlation — a much shakier "did you just make this up" risk than the existing rate-card/monsoon/BCA-grounded references.
...
### Live demo script (`ml-experiments/demo_scenarios.py`) — scaffolded early on Phase 1d's branch
- **Why now, not Day 5**: nominally `feature/ml-5-demo-prep` (Day 5 PM) scope per `dynamic-pricing-execution-plan.md`, but pulled forward onto the Phase 1d branch — same "get ahead of schedule when it's low-risk" pattern as Phase 1c. Low-risk because it uses placeholder asset data and only exercises the six existing Phase 1b features (not `period_utilization`/`lead_time_days`), so it doesn't depend on anything else in that branch.
- **Why a script at all**: `predict_price(...)` is deliberately in-process only (§5.1/§3 of the spec, locked) — no HTTP endpoint, no Postman option for a lecturer to demo against. A script is the only way to show it live.
- **Deliberately narrow, not a substitute for existing coverage**: 1–2 illustrative scenario pairs (condition effect, duration effect) for a non-technical live audience, holding all other inputs fixed per pair. Explicitly not exhaustive — `tests/` still owns plumbing-correctness assertions (shape/type, clamping boundaries, NaN handling), and `shap_review.py` still owns comprehensive directional-sweep coverage across all categories/feature ranges. Conflating any of these three would either bloat the demo script past what a live audience needs, or leave a real coverage gap disguised as "we have a demo for that."
- **Follow-up idea, not scheduled**: extend `shap_review.py` with a coverage table (features × categories, PASS/FAIL + violation % per cell, reusing `category_metrics.py`'s table-rendering pattern). Worth doing — today's directional sweeps only run against one frozen representative row per feature (dataset-mode category), the same blind spot that let the `platform_height` gap hide behind a healthy overall metric in Phase 1b. Best done once Phase 1d's two new features exist too, so the table covers all 8 features from the start rather than needing a second pass.
