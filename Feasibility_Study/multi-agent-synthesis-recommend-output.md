...
**Building blocks as-built (S7.0–S7.4):** `RecommendAgentState` + F-2 validation (`app/agents/recommend_state.py`); allowlisted tools via `fleet_tools` + `tool_factory`; recommend DAG (`app/agents/recommend_graph.py` / `recommend_nodes.py`); **stub Coordinator [8]** (`app/agents/recommend_synthesis.py`). HTTP Call 2 enrich remains **S7.5**.
