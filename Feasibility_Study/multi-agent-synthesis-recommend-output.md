...
**Building blocks as-built (S7.0–S7.7):** `RecommendAgentState` + F-2 validation (`app/agents/recommend_state.py`); allowlisted tools via `fleet_tools` + `tool_factory`; **S7.2 Neo4j tools** (`neo4j_cypher_read` templates + `trigger_neo4j_populate` no-op; K-3 skip); recommend DAG (`app/agents/recommend_graph.py` / `recommend_nodes.py`); **stub Coordinator [8]** (`app/agents/recommend_synthesis.py`); Call 2 HTTP enrich behind `RECOMMEND_VIA_AGENT_GRAPH` (`SessionRecommendService`); G-1 `tool_traces` (`recommend_traces.py`); **A–L prompts** (`app/agents/recommend_prompts.py`) + `build_recommend_runtime` tool DI. Live Neo4j populate remains **S8**.
