...
The production trainer wrote the tracked candidate `app/services/pricing/artifacts/model_v2.pkl` and `current_v2.json`. Holdout MAE/RMSE/R² are `16.6376`/`26.1103`/`0.9866`. At Phase 2d-ii/iii completion the serving loader still read only the unchanged `model.pkl`/`current.json`; the separately reviewed Phase 2e promotion later copied v2 onto those serving filenames.
...
Final convergence on branch `HR-177-ml-promote-calibrated-model-to-production` passed the Phase 2e focused tests, scoped Ruff, and diff hygiene. The repository default suite finished with 420 passed and 5 skipped.
