...
| Field | Value |
|-------|--------|
| **Status** | Phase 2a **implemented and verified (2026-08-11)** — `app/services/pricing/` package built, category-name mapping fixed, real per-asset guardrail clamping live. Phase 2c **implemented and verified (2026-08-11)** — `POST /internal/v1/pricing/quote` live, reusing `predict_price(...)`, 5 new tests, 149 total passing. Phase 2b (lean pipeline wiring) **implemented and verified (2026-08-11)** — `pricing_client.py` swapped to `app.services.pricing.model.predict_price(...)`, `item.pricing.daily_rate` now production-model-backed end to end, 154 total passing. Phase 2d-i **implemented and live-verified (2026-08-12)** — all 27 real asset bounds measured without changing production data or artifacts; Phase 2d-ii/iii remain. Phase 3 (real-data blend + scheduled retrain) not yet implemented |
