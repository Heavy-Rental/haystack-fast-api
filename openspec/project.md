...
**Target (later):** I2 production default `pgvector`; production default `RECOMMEND_VIA_AGENT_GRAPH`; align config sync table names with haystack ORM; **S8.3** live Neo4j tools; Naive/hybrid RAG over manuals; async ML training. Phase 7 **S7.0–S7.7 as-built**. **S4 as-built.** **S8.1–S8.2 as-built (config):** `neo4j-populate` + post-sync/admin trigger. Normative detail lives in capability specs—not here.
