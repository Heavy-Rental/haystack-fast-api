...
**Implementation shape**: one shared resolver (e.g. `resolve_pricing_schema()` / a `PricingReadSession` wrapper) that both `pricing_repository.py`'s `compute_period_utilization()` and the guardrail-bound `Asset` read (§5.4) depend on, so the source decision is made once per call and applied consistently — not two independent fallback implementations at two call sites. In scope for **Phase 1e** (§7), not deferred to Phase 2 — Phase 1e is already where the first read-only models are introduced, and building the shared resolver there is cheaper than retrofitting it once Phase 2 adds the guardrail-bound read.
...
1. `feature/ml-3-pricing-service` (Day 4): scaffold `app/services/pricing/`, port `feature_schema.py` (already includes `period_utilization`/`lead_time_days`/`spec_band()` from Phase 1d), implement `model.py`/`train.py`, guardrail clamping, minimal SQLAlchemy read models (§5.3 — confirm schema first; relocates, doesn't rebuild, Phase 1e's `pricing_repository.py`, including its shared read-resilience resolver, §5.3.1 — the guardrail-bound `Asset` read must go through the same resolver, not a second fallback implementation).
